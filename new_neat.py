...
# AI Agent that plays Tetris using heuristic weights
class TetrisAI:
    ...
    def evaluate_board(self, game):
        """Evaluate current board state using heuristics"""
        grid = game.grid.grid
        ...
        # Calculate holes (empty cells with filled cells above)
        holes = 0
        for col in range(game.grid.num_cols):
            block_found = False
            for row in range(game.grid.num_rows):
        ...
        # Lines cleared is tracked separately in game
        lines_cleared = game.score / 10  # Rough estimate
    ...
    def get_best_move(self, game):
        """Try all possible moves and return the best one"""
        ...
        for rotation in range(4):
            ...
            for col in range(-2, game.grid.num_cols + 2):
                ...
                # Check if position is valid before dropping
...
def main():
    ...
    # Training mode - run fast by default
    speed = 50  # Games per frame - much faster!
    ...
    while True:
        ...
        for _ in range(speed):
            if game.game_over:
                ...
                # Move to next AI or next generation
